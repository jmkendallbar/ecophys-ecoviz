...
def generate_features(path_to_edf, output_csv_path=None, config_file_path=None, add_yasa_features=True):
    ...
    # print('starting delta power')
    # eeg_data = raw.get_data([config['EEG Channel']])[0].copy()
    # # EEG spectral power of delta frequency [0.4, 1]
    # eeg_low_delta_power = seal_fe.get_rolling_band_power_welch(eeg_data, start_index, end_index, freq_range=(0.4, 1),
    #                                                        ref_power=config['Delta Power Reference Power'], freq=sfreq,
    #                                                        window_sec=config['Delta Power Calculation Window'],
    #                                                        step_size=config['Step Size'])
    # # EEG spectral power of delta frequency [1, 4]
    # eeg_high_delta_power = seal_fe.get_rolling_band_power_welch(eeg_data, start_index, end_index, freq_range=(1, 4),
    #                                                        ref_power=config['Delta Power Reference Power'], freq=sfreq,
    #                                                        window_sec=config['Delta Power Calculation Window'],
    #                                                        step_size=config['Step Size'])
    # eeg_delta_power = eeg_low_delta_power + eeg_high_delta_power
    # print('starting theta power')
    # # EEG spectral power of delta frequency [4, 8]
    # eeg_theta_power = seal_fe.get_rolling_band_power_welch(eeg_data, start_index, end_index, freq_range=(4, 8),
    #                                                        ref_power=config['Delta Power Reference Power'], freq=sfreq,
    #                                                        window_sec=config['Delta Power Calculation Window'],
    #                                                        step_size=config['Step Size'])
    # print('done', end='\n\n' + '-'*50 + '\n')
    # print('starting alpha power')
    # # EEG spectral power of delta frequency [8, 12]
    # eeg_alpha_power = seal_fe.get_rolling_band_power_welch(eeg_data, start_index, end_index, freq_range=(8, 12),
    #                                                        ref_power=config['Delta Power Reference Power'], freq=sfreq,
    #                                                        window_sec=config['Delta Power Calculation Window'],
    #                                                        step_size=config['Step Size'])
    # print('done', end='\n\n' + '-'*50 + '\n')
    # print('starting sigma power')
    # # EEG spectral power of delta frequency [12, 16]
    # eeg_sigma_power = seal_fe.get_rolling_band_power_welch(eeg_data, start_index, end_index, freq_range=(12, 16),
    #                                                        ref_power=config['Delta Power Reference Power'], freq=sfreq,
    #                                                        window_sec=config['Delta Power Calculation Window'],
    #                                                        step_size=config['Step Size'])
    # print('done', end='\n\n' + '-'*50 + '\n')
    # print('starting beta power')
    # # EEG spectral power of delta frequency [16, 30]
    # eeg_beta_power = seal_fe.get_rolling_band_power_welch(eeg_data, start_index, end_index, freq_range=(16, 30),
    #                                                        ref_power=config['Delta Power Reference Power'], freq=sfreq,
    #                                                        window_sec=config['Delta Power Calculation Window'],
    #                                                        step_size=config['Step Size'])
    # print('done', end='\n\n' + '-'*50 + '\n')
    # print('starting zero crossings')
    # # EEG zero crossings per second
    # eeg_zero_crossings = seal_fe.get_rolling_zero_crossings(eeg_data, start_index, end_index, freq=sfreq,
    #                                                         window_sec=config['Zero Crossings Calculation Window'])
    # print('done', end='\n\n' + '-'*50 + '\n')
    # print('starting absolute power')
    # # EEG absolute spectral power on frequency range [0.4, 30]
    # eeg_absolute_power = seal_fe.get_rolling_band_power_welch(eeg_data, start_index, end_index, freq_range=(0.4, 30),
    #                                                           ref_power=config['Absolute Power Reference Power'], freq=sfreq,
    #                                                           window_sec=config['Absolute Power Calculation Window'],
    #                                                           step_size=config['Step Size'])
    # print('done', end='\n\n' + '-'*50 + '\n')
    ...
    if add_yasa_features == True:
        ...
        eeg_data = raw.get_data([config['EEG Channel']])[0].copy()
        yasa_features_eeg = seal_fe.get_features_yasa(eeg_data, start_index, end_index, freq_broad=(0.4, 30), freq=500,
